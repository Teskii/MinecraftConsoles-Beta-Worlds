...
#include "BetaTreeFeature.h"
#include "BetaTreeSupport.h"

BetaTreeFeature::BetaTreeFeature(bool doUpdate) : Feature(doUpdate)
{
}

bool BetaTreeFeature::place(Level *level, Random *random, int x, int y, int z)
{
    const int treeHeight = random->nextInt(3) + 4;
    bool free = true;
    ...
    for (int yy = y - 3 + treeHeight; yy <= y + treeHeight; ++yy)
    {
        ...
        for (int xx = x - radius; xx <= x + radius; ++xx)
        {
            ...
            for (int zz = z - radius; zz <= z + radius; ++zz)
            {
                ...
                {
                    PlaceBetaWorldgenBlock(level, xx, yy, zz, Tile::leaves_Id, 0);
                }
            }
        }
    }
    ...
    for (int yy = 0; yy < treeHeight; ++yy)
    {
        ...
        if (tile == 0 || tile == Tile::leaves_Id)
        {
            PlaceBetaWorldgenBlock(level, x, y + yy, z, Tile::treeTrunk_Id, 0);
        }
    }
